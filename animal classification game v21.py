...
def colour_schemes():
    ...
    if language_options==True: #if language options are open, close them
        options.place_forget()
        language_options=False
    #display 4 colour option buttons (dark, light, high con) 
    colour1=Button(root, text="dark mode", command=colour1_initial, font=("Georgia", 10), bg='#dbb4b4')
    colour1.bind("<Button-1>", colour1)
    colour1.place(x=750, y=95)
    colour2=Button(root, text="light mode", command=colour2_initial, font=("Georgia", 10), bg='#dbb4b4')
    colour2.bind("<Button-1>", colour2)
    colour2.place(x=750, y=125)
    colour3=Button(root, text="High Contrast", command=colour3_initial, font=("Georgia", 10), bg='#dbb4b4')
...
def update_widget_colour():
    ...
    if contrast_mode==False and dark_mode==False and light_mode== False:
        pass
    if dark_mode==True or (light_mode==False and contrast_mode==False):
        colour1_change()
    if light_mode==True or (dark_mode==False and contrast_mode==False):
        colour2_change()
    if contrast_mode==True or (light_mode==False and dark_mode==False):
        colour3_change()
...
def scale_widget(scale, widget):
    ...
    widget.place_configure(width=int(current_width * scale), height=int(current_height * scale))
    ...
    if hasattr(widget, "image"):
        ...
        if scale==0.5:
            image_resize=image_resize.configure(size=(900, 500))
            print("done")
