...
def file():
    ...
    global colour2
    global colour1
    if language_options==False and size_options==False and colour_options==False: #make sure all options are closed first (colour_options==False and size_options==False and)
...
def text_size():
    ...
    T50=Button(root, text="-", font=("Georgia", 15), bg='#b4c3db', 
               command= lambda scale=0.2: scale_all_widgets(scale))
    T50.place(x=700, y=35)
    T200=Button(root, text="+", font=("Georgia", 15), bg='#b4c3db', 
                command= lambda scale=1.5: scale_all_widgets(scale))
    T200.place(x=700, y=80)
    size_options=True
...
def scale_widget(scale, widget):
    ...
    if isinstance(widget, Frame):
        for widget in widget.winfo_children():
            if widget_has_image(widget):
                return
    elif widget_has_image(widget):
        return

    def widget_has_text(widget):
        try:
    ...
    if widget_has_text(widget):
        if len(widget.cget("text"))>1:

            # Retrieve the font configuration of the label
            current_font = widget.cget("font")

            # To get detailed font attributes, you can use the Font class
